...
pub struct SphereIntersectionParams<T: GFloat> {
    pub t_ray: T,
...
impl<T: GFloat> Bounded3<T> for Sphere<T> {
...
impl<T: GFloat> Sphere<T> {
    pub fn new(radius: T, z_min: T, z_max: T, phi_max: T) -> Self {
        assert!(z_min < z_max);

        let theta_min = T::clamp(z_min / radius, T::zero(), T::one()).acos();
        let theta_max = T::clamp(z_max / radius, T::zero(), T::one()).acos();
...
impl<T: GFloat> Sphere<T> {
    fn intersect_ray(&self, ray: Ray<T>) -> Option<SphereIntersectionParams<T>> {
        ...
        let b = Vector3::from(ray.origin).dot(ray.direction) * T::from_i64(2);
        let c = Vector3::from(ray.origin).square_norm() - self.radius.square();

        let (t0, t1) = solve_quadratic(a, b, c)?;

        let get_intersection = |t: T| -> Option<SphereIntersectionParams<T>> {
            if !t.in_range(T::zero(), ray.tmax) {
                return None;
            }
...
impl<T: GFloat> IntersectsRay<T> for Sphere<T> {
    ...
    fn intersect(&self, ray: Ray<T>) -> Option<IntersectionResult<T>> {
        ...
        let theta = T::clamp(p.z() / self.radius, T::zero(), T::one()).acos();
        ...
        let phi_max_plus_pi = self.phi_max + T::pi();
        let theta_max_minus_theta_min = self.theta_max - self.theta_min;

        let u = (phi + T::pi()) / phi_max_plus_pi;
        ...
        // which is
        let dpdu = Vector3::new(-p.y(), p.x(), T::zero()) * phi_max_plus_pi;
        let dpdv = Vector3::new(p.z() * cos_phi, p.z() * sin_phi, -self.radius * sin_theta)
            * (theta_max_minus_theta_min);

        let surface_interaction =
            SurfaceInteraction::new(intersection_params.p, Point2::new(u, v), dpdu, dpdv);
...
impl<T: GFloat> Shape<T> for Sphere<T> {}

// Should this be here?
fn solve_quadratic<T: GFloat>(a: T, b: T, c: T) -> Option<(T, T)> {
    // discriminant
    let d = b.square() - a * c * T::from_i64(4);
    if d.is_nonnegative() {
        let sqrt_d = d.sqrt();
...
mod tests {
    ...
    fn test_ray_full_sphere_intersection() {
        ...
        let dist_phi = Uniform::new_inclusive(-f64::pi(), f64::pi());
        let dist_theta = Uniform::new_inclusive(0.0, f64::pi());
        ...
        for _ in 0..num_tests {
            ...
            let (z_min, z_max) = f64::min_max(dist_z.sample(&mut rng), dist_z.sample(&mut rng));
            let phi_max = dist_phi.sample(&mut rng);

            let sphere = Sphere::<f64>::new(radius, z_min, z_max, phi_max);
            let spherei = Sphere::<Interval<f64>>::new(
                Interval::from(radius),
                Interval::from(z_min),
                Interval::from(z_max),
                Interval::from(phi_max),
            );
            ...
            let p0i = Point3::<Interval<f64>>::from(p0);
            ...
            let p1i = Point3::<Interval<f64>>::from(p1);
            ...
            let rayi = Ray::new(origini, diri, Interval::one());

            let intersection = sphere.intersect(ray);
            let intersectioni = spherei.intersect(rayi);
            ...
            if p0_on_sphere {
                ...
                assert!(intersectioni.unwrap().t_ray.almost_eq(t_offset));
            } else {
            ...
            let t_offset = dist_offset.sample(&mut rng);
            let origin = p0 - dir * t_offset;
            let ray = Ray::new(origin, dir, 1.0 + 2.0 * t_offset);

            let origini = p0i - diri * t_offset;
            let rayi = Ray::new(origini, diri, Interval::one() + t_offset * 2.0);
            ...
            if p0_on_sphere || p1_on_sphere {
                ...
                if p0_on_sphere {
                    ...
                    assert!(intersectioni.unwrap().t_ray.almost_eq(1.0 + t_offset));
                }
            } else {
                assert!(intersection.is_none());
            ...
            let t_offset = dist_offset.sample(&mut rng);
            let origin = p0 + dir * t_offset;
            let ray = Ray::new(origin, dir, 1.0 - t_offset - 0.1);

            let origini = p0i + diri * t_offset;
            let rayi = Ray::new(origini, diri, Interval::from(1.0 - t_offset - 0.1));
